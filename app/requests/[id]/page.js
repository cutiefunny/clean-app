      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (requestData.applicantName !== authData.name || requestData.applicantContact !== authData.phoneNumber) {
            ...
            // [수정] Firestore의 'requestDate' 필드를 'usageDate'로 매핑합니다.
            const usageDate = requestData.requestDate?.toDate 
              ? requestData.requestDate.toDate().toLocaleDateString('ko-KR') 
              : '날짜 정보 없음';

            setRequest({
              id: docSnap.id,
              serviceType: requestData.field || '정보 없음',
              usageDate: usageDate,
